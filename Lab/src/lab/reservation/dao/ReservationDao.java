...
public class ReservationDao {
	...
	public List<Reservation> select(Connection conn, Date startdate) throws SQLException{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		
		try {
			pstmt = conn.prepareStatement("select * from reservation where startdate = ?");
	...
	public List<Reservation> selectMsg(Connection conn, Date startdate) throws SQLException{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		
		try {
			pstmt = conn.prepareStatement("select * from reservation where startdate = ? and approval = '예약승인' ");
			pstmt.setDate(1, startdate);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				List<Reservation> dateList = new ArrayList<Reservation>();
	...
	public int updatepermisson(Connection conn, int sno, String labRoom,  boolean flag) throws SQLException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String status = "";
		int i = 0;
